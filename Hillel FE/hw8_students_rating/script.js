...
//Average mark
const averageMark = test.forEach(student => {
    let average = 0;
...
//Median mark
const medianMark = test.forEach(student => {
    student.marks.sort();
    let medianMark;
...
const newStudent = () => {
    test.push({
        ...
        specialty: 'International Tactics Facilitator',
        marks: [32, 51, 30, 83, 46, 45, 26, 67, 44, 30, 69, 89, 25, 64, 65, 62, 86, 96, 75, 38, 47, 83, 26, 45, 72] 
    })
    return test;
}
...
const sortByAverageMark = test => {
    test.sort(averageMarkSort);
    test.forEach((student) => {
        return console.log(student.name + ' - ' + student.averageMark);
    });
    const removeUndefined = test.filter(student => typeof student !== 'undefined');
...
const topFiveStudents = test => {
    test.sort(averageMarkSort);
    const topFiveStudents = test.splice(0, 5);
